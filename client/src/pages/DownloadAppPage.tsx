...
import { Smartphone, Download, CheckCircle, Clock, Monitor, Apple, AlertTriangle, Code } from 'lucide-react'
import api from '../services/api'

export default function DownloadAppPage() {
  const [downloading, setDownloading] = useState<string | null>(null)

  const handleDownload = async (platform: 'android' | 'windows' | 'ios', type: 'release' | 'snapshot' = 'release') => {
    setDownloading(`${platform}-${type}`)
    try {
      if (platform === 'android') {
        const endpoint = type === 'snapshot' ? '/download/android/snapshot' : '/download/android'
        ...
        link.href = url
        link.setAttribute('download', type === 'snapshot' ? 'Cownect-Snapshot.apk' : 'Cownect-Android.apk')
        document.body.appendChild(link)
        link.click()
        link.remove()
        window.URL.revokeObjectURL(url)
      } else if (platform === 'windows') {
        // Para Windows - próximamente
        alert('La aplicación para Windows estará disponible próximamente')
      } else if (platform === 'ios') {
        // Para iOS - próximamente
        alert('La aplicación para iOS estará disponible próximamente')
      }
    } catch (error: any) {
      console.error('Error al descargar:', error)
      const errorMessage = error.response?.data?.message || 'Error al descargar la aplicación. Por favor intenta más tarde.'
      alert(errorMessage)
    } finally {
      setDownloading(null)
    }
      ...
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center justify-center w-24 h-24 bg-black rounded-3xl shadow-2xl border border-black overflow-hidden p-2 mb-6">
            <img 
              src={`${import.meta.env.BASE_URL}images/logo_front.jpeg`} 
              alt="Cownect Logo" 
          ...
          <p className="text-xl text-gray-700 font-normal">
            Instala la aplicación en tu dispositivo y gestiona tu ganado desde cualquier lugar
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 border border-black">
          <h2 className="text-2xl font-bold text-black mb-6 text-center">Instrucciones de Instalación</h2>
          <div className="space-y-4">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-gray-700 font-normal">Haz clic en el botón "Descargar APK" para Android</p>
              </div>
            </div>
            <div className="flex items-start gap-4">
              <div className="bg-black text-white rounded-full w-8 h-8 flex items-center justify-center font-bold flex-shrink-0">
              ...
              <div>
                <h3 className="font-bold text-black mb-1">Permite instalación de fuentes desconocidas</h3>
        ...
        <div className="mt-8 bg-white rounded-2xl shadow-xl p-8 border-2 border-orange-300">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h2 className="text-2xl font-bold text-black">Snapshots (Solo Android)</h2>
          </div>
          <div className="bg-orange-50 border-2 border-orange-200 rounded-xl p-4 mb-6">
            <div className="flex items-start gap-3">
              <AlertTriangle className="h-6 w-6 text-orange-600 flex-shrink-0 mt-0.5" />
              <div>
                <p className="text-orange-900 font-bold mb-2">⚠️ ADVERTENCIA IMPORTANTE:</p>
                <ul className="text-orange-800 text-sm space-y-1 list-disc list-inside">
                  ...
                  <li><strong>NO tienen acceso a la base de datos del servidor</strong></li>
        ...
        <div className="mt-8 bg-yellow-50 border-2 border-yellow-300 rounded-xl p-6">
          <p className="text-yellow-800 font-semibold text-center">
            ⚠️ Nota: La aplicación Android se descarga directamente desde nuestro servidor. 
            Asegúrate de tener activada la opción de "Fuentes desconocidas" en tu dispositivo.
          </p>
        </div>
      </div>
    </div>
